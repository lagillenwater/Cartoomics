...
from create_subgraph import subgraph_shortest_path,subgraph_shortest_path_pattern
...
def one_path_search(input_df,s,igraph,igraph_nodes,labels_all,edgelist,search_type,triples_file,output_dir,kg_type,manually_chosen_uris):
    ...
    exists = check_for_existance(source_name,target_name,output_dir)

    if exists == 'false':

        subgraph_sp,manually_chosen_uris = subgraph_shortest_path(input_df,igraph,igraph_nodes,labels_all,edgelist,False,search_type,kg_type,manually_chosen_uris)

        #print("Outputting CS visualization......")

        if len(subgraph_sp) > 0:
            cs_noa_df = output_visualization(input_df,source_name,target_name,subgraph_sp,output_dir)

    return manually_chosen_uris
