...
def get_uri(labels,value, kg_type,s,manually_chosen_uris):
    ...
    if kg_type == 'pkl' or kg_type == 'mikg4md':
        for i in range(len(df)):
            ...
            try:
                ...
                o_label = [labels_all.loc[labels_all['entity_uri'] == df.iloc[i].loc['O'],'label'].values[0]]
                #df.iloc[i].loc['O'] = labels_all.loc[labels_all['entity_uri'] == df.iloc[i].loc['O'],'label'].values[0]
            data.append(s_label[0])
            data.append(p_label[0])
            data.append(o_label[0])
...
def find_shortest_path(start_node,end_node,graph,g_nodes,labels_all,triples_df,weights,search_type, kg_type,s,manually_chosen_uris):

    #Remove if given uris already
    #node1,manually_chosen_uris = get_uri(labels_all,start_node,kg_type,s,manually_chosen_uris)
    #node2,manually_chosen_uris = get_uri(labels_all,end_node,kg_type,s,manually_chosen_uris)
